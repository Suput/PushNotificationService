...
final class TopicController {
    ...
    func createTopic(_ req: Request) throws -> EventLoopFuture<TopicServer> {
        let topicClient = try req.content.decode(TopicClient.self)

        return TopicNotification.query(on: req.db).filter(\.$nameTopic == topicClient.name).first()
            .flatMap { (topic) -> EventLoopFuture<TopicServer> in
            guard let topicDB = topic else {
                let topicDB = TopicNotification(topic: topicClient.name)
                return topicDB.create(on: req.db).map {
                    TopicServer(id: topicDB.id!, topicName: topicDB.nameTopic)
                }
            }
    ...
    func subscribeTopic(_ req: Request) throws -> EventLoopFuture<HTTPStatus> {
        ...
        let getTopicDB = TopicNotification.query(on: req.db).filter(\.$id == topic.topicID).first()
            .unwrap(or: Abort(.notFound))

        return UserDevices.query(on: req.db).filter(\.$id == topic.userID).first()
            .unwrap(or: Abort(.notFound)).and(getTopicDB).flatMap { (data) -> EventLoopFuture<HTTPStatus> in

            data.0.$topics.isAttached(to: data.1, on: req.db).flatMap { (isAttached) -> EventLoopFuture<HTTPStatus> in
                if isAttached {
